          ...
          <div key={r.id} className="card p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-ink-500">on {r.product?.name} · {date(r.created_at)}</div>
              </div>
            ...
            <div className="mt-3 flex gap-2">
              ...
              {r.status !== 'approved' && <button onClick={() => setS(r, 'approved')} className="btn-outline !py-1 !px-2 text-xs">Approve</button>}
              {r.status !== 'rejected' && <button onClick={() => setS(r, 'rejected')} className="btn-outline !py-1 !px-2 text-xs">Reject</button>}
              <button onClick={() => del(r)} className="btn-ghost !py-1 !px-2 text-xs text-rose-600">Delete</button>
            </div>
          </div>
        ))}
      </div>
