...
export default function Checkout() {
  ...
  return (
    <div className="container-narrow py-8 sm:py-12">
      ...
      <h1 className="mt-2 display-1">Checkout</h1>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_400px] lg:gap-10">
        <div className="space-y-6 min-w-0">
          <section className="card p-5 sm:p-6">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {addresses.map((a) => (
                  <button
                    key={a.id}
                    type="button"
                    onClick={() => { setSelectedAddr(a.id); setForm(a) }}
                    className={`rounded-xl border p-4 text-left text-sm transition ${
                      selectedAddr === a.id ? 'border-ink-900 bg-ink-50' : 'border-ink-200 hover:border-ink-400'
                    }`}
                  >
                ...
                <button
                  type="button"
                  onClick={() => { setSelectedAddr(null); setForm({ ...form, full_name: '', line1: '', city: '', postal_code: '' }) }}
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2"><div className="label">Postal code</div><input className="input" value={form.postal_code} onChange={(e) => setForm({ ...form, postal_code: e.target.value })} /></div>
            </div>
          </section>
          ...
          <section className="card p-5 sm:p-6">
            ...
            <p className="mt-3 inline-flex items-center gap-1.5 text-[11px] uppercase tracking-luxe text-ink-500">
              <Lock className="h-3 w-3" /> UI preview — real Stripe.js integration is wired on the backend.
            </p>
          </section>

          <section className="card p-5 sm:p-6">
            <h2 className="font-display text-xl font-semibold">Order notes</h2>
            <textarea
              rows={3}
        ...
        <aside className="card h-fit p-6 lg:sticky lg:top-28">
          ...
          <div className="mt-4 space-y-3 max-h-72 overflow-y-auto pr-1 sm:max-h-none">
            {items.map((i, idx) => (
              <div key={idx} className="flex gap-3 text-sm">
                <img src={i.image} alt="" className="h-16 w-14 flex-shrink-0 rounded-md object-cover" />
                <div className="min-w-0 flex-1">
                  <div className="truncate font-semibold">{i.name}</div>
                  <div className="mt-0.5 text-[11px] uppercase tracking-luxe text-ink-500">
                    {[i.size, i.color].filter(Boolean).join(' · ')} · Qty {i.quantity}
                  </div>
                </div>
                <div className="font-semibold whitespace-nowrap">{money(i.price * i.quantity)}</div>
              </div>
            ))}
          </div>
          ...
          {appliedCoupon && (
            <div className="mt-2 text-xs text-emerald-700">
              ✓ {appliedCoupon.code} applied — saved {money(appliedCoupon.discount)}
            </div>
          )}

          <div className="rule my-5" />
          <dl className="space-y-2.5 text-sm">
            <div className="flex justify-between"><dt>Subtotal</dt><dd className="font-medium">{money(totals.subtotal)}</dd></div>
            {totals.discount > 0 && (
              <div className="flex justify-between text-emerald-700"><dt>Discount</dt><dd>−{money(totals.discount)}</dd></div>
            )}
            <div className="flex justify-between"><dt>Shipping</dt><dd className="font-medium">{totals.shipping > 0 ? money(totals.shipping) : 'Free'}</dd></div>
            <div className="flex justify-between"><dt>Tax</dt><dd className="font-medium">{money(totals.tax)}</dd></div>
            <div className="rule" />
            <div className="flex justify-between font-display text-lg font-semibold"><dt>Total</dt><dd>{money(totals.total)}</dd></div>
          </dl>
          ...
          <p className="mt-3 text-center text-[10px] uppercase tracking-luxe text-ink-400">
