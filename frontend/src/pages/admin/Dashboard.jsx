  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-600">Snapshot of your store performance.</p>
      </div>
      ...
      {t.low_stock > 0 && (
        <div className="flex items-center gap-3 rounded-2xl bg-amber-100 p-4 text-sm text-amber-900">
          <AlertTriangle className="h-5 w-5" /> {t.low_stock} product(s) running low on stock. <Link className="ml-auto underline" to="/admin/products">Review</Link>
        </div>
      )}
          ...
          <ul className="mt-3 divide-y divide-ink-100">
            {d.top_products.map((p) => (
              <li key={p.id} className="flex items-center justify-between py-2 text-sm">
                <span>{p.name}</span>
                <span className="text-ink-500">{p.sold_count} sold · {money(p.price)}</span>
              </li>
            ))}
          </ul>
        </div>
          ...
          <ul className="mt-3 divide-y divide-ink-100">
            {d.recent_orders.map((o) => (
              <li key={o.id} className="flex items-center justify-between py-2 text-sm">
                <Link to={`/admin/orders/${o.id}`} className="font-mono hover:underline">{o.order_number}</Link>
    ...
    <div className="flex h-32 items-end gap-1">
      {data.map((d, i) => (
        <div key={i} className="flex-1 group relative">
          <div className="rounded-t bg-accent-300 transition group-hover:bg-accent-500" style={{ height: `${(Number(d.total) / max) * 100}%` }} />
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded bg-ink-900 px-2 py-1 text-[10px] text-white opacity-0 group-hover:opacity-100">
            {d.d}: {money(d.total)}
          </div>
        </div>
      ))}
